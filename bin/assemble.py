...
def run(out, outdir, command, mv = False, special = False, silent = False):
	...
	if check(out) is False or special is True:
...
def velveth(out, kmer, paired, single, unused_reads, silent):
	...
	vh = ['velveth %s %s' % (out, kmer)]
		...
		for count, file in enumerate(reads[0]):
			...
			if type is False:
				continue
			list.append('-%s -%s %s' % (type, cat, file))
		list = ' '.join(list)
		vh.append(list)
...
def velvet(paired = [], single = [], out = 'velvet', kmer_min = 21, kmer_max = 71, kmer_increase = 10, min_contig = 400, exp_cov = False, cov_diff = 100, max_coverage = False, cov_cutoff = 'auto', unused_reads = False, read_trkg = False, scaffolding = True, silent = False):
	...
	for kmer in range(kmer_min, kmer_max)[::kmer_increase]:
		...
		if exp_cov is False:
			...
			paired, single = velvetg(paired, single, out, out_file, vh, kmer, exp_cov, max_coverage, cov_cutoff, unused_reads, read_trkg, min_contig, scaffolding, silent)
		else:
			min, max = exp_cov - cov_diff, exp_cov + 2*cov_diff
			if min == max:
				cov = exp_cov
				out_file = '%s/kmer_%s_cov_%s.fasta' % (out, kmer, cov)
				paired, single = velvetg(paired, single, out, out_file, vh, kmer, cov, max_coverage, cov_cutoff, unused_reads, read_trkg, min_contig, scaffolding, silent)
